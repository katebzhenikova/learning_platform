...
class MaterialCreateAPIViewTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.test1 = Test.objects.create(
            question="Test 1", material=self.material, owner=self.teacher
        )
        self.correct_answer = AnswerOption.objects.create(
            answer_text="Correct", is_correct=True, test=self.test1
        )
        self.incorrect_answer = AnswerOption.objects.create(
            answer_text="Incorrect", is_correct=False, test=self.test1
        )
    ...
    def test_student_cannot_create_answer_with_wrong_option(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(
            response.data["is_correct"], False
        )  # Проверяем, что is_correct равно False
